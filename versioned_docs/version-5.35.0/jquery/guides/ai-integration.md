...
| Framework | Name | URL |
|:---|:---|:---|
| jQuery | Mobiscroll jQuery | <DocsUrl path="5.35.0/llms-jquery.txt" /> |
...
**Fix:** Verify that you have the correct `.mdc` file in place. For jQuery, use `mobiscroll-jquery.mdc`, not a different framework's `.mdc` file. In Cursor, check that the registered @docs source points to `5.35.0/llms-jquery.txt`.
