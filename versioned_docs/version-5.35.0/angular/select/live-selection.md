---
sidebar_position: 5
sidebar_label: Live selection
displayed_sidebar: angularSidebar
description: 'Enable live selection in the Mobiscroll Select for Angular — fires change immediately as the user scrolls through options, before confirmation.'
---

import Content from '../../_shared/live_selection.mdx';

# Live selection

<Content />

Furthermore the [`onTempChange`](./api#event-onTempChange) event is raised whenever the temporary value changes.
