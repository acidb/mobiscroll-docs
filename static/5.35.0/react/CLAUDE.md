# Mobiscroll Documentation — AI Context (React, v5)
...
### Primary: Full React v5 docs (single file, all content inline)

- **React v5:** {{DOCS_BASE_URL}}/5.35.0/llms-react-full.txt

### Secondary: React v5 table of contents (links to individual .md pages)

- **React v5:** {{DOCS_BASE_URL}}/5.35.0/llms-react.txt

### Individual .md pages

Pattern: `{{DOCS_BASE_URL}}/react/{section}/{page}.md`
Example: `{{DOCS_BASE_URL}}/react/eventcalendar/overview.md`
...
- **Icons:** {{DOCS_BASE_URL}}/5.35.0/llms-icons.txt

---

## 2. Content Selection Priority

1. `5.35.0/llms-react-full.txt` — complete v5 docs in one file (best for full context)
2. Individual `.md` pages — for targeted lookups
3. `5.35.0/llms-react-full.txt` — table of contents with links to .md pages
...
**This project uses React and Mobiscroll v5. Do not detect or switch frameworks. Always use `5.35.0/llms-react-full.txt`.**
...
→ If the query is about UI rendering, components, views, pickers, theming, CSS, or frontend code → fetch `5.35.0/llms-react-full.txt`.

→ If the query is about **displaying** Google Calendar / Outlook / Apple Calendar events in the Eventcalendar UI (client-side data binding, `data` prop, event loading) — this is a **UI question**. Fetch `5.35.0/llms-react-full.txt`.
...
3. Show a minimal Eventcalendar example for React v5, passing the fetched data via the `data` prop.
...
- If the user explicitly asks about a different Mobiscroll framework, respond: "This context is configured for React v5 (`@mobiscroll/react`). For [other framework], use the appropriate CLAUDE.md instead."
...
- Displaying Google Calendar / Outlook / Apple Calendar events inside the Eventcalendar UI component (client-side) → React v5 docs (`5.35.0/llms-react-full.txt`)
...
| Displaying Google Calendar events in Eventcalendar (client-side) | `5.35.0/llms-react-full.txt` |
...
4. **Fetch failure fallback.** If `5.35.0/llms-react-full.txt` is unreachable, fall back to individual `.md` pages (Priority 2), then the TOC file (Priority 3). If all sources fail, state that docs are unavailable and ask the user to share relevant doc content.
...
9. **Package:** `@mobiscroll/react`. CSS: `import '@mobiscroll/react/dist/css/mobiscroll.min.css'`.
...
| Using Mobiscroll Connect docs for frontend UI | Connect = server REST API. Use `5.35.0/llms-react-full.txt` for UI. |
