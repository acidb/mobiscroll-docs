...
| Framework | Name | URL |
|:---|:---|:---|
| JavaScript | Mobiscroll JavaScript | <DocsUrl path="llms-javascript.txt" /> |
...
**Fix:** Verify that you have the correct `.mdc` file in place. For JavaScript, use `mobiscroll-javascript.mdc`, not a different framework's `.mdc` file. In Cursor, check that the registered @docs source points to `llms-javascript.txt`.
