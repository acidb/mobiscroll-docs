...
We slightly changed the structure of the [marked](/react/eventcalendar/api#opt-marked), [labels](/react/eventcalendar/api#opt-labels) and [colors](/react/eventcalendar/api#opt-colors) objects. To specify the date, use the `date` property, or the `start` and `end` properties for multiple days, and use the `recurring` property to specify multiple occurrences.
...
The `data` option only supports an array of items with the `text`, `value`, `group` and `disabled` properties. HTML content for options is supported from version 5.12.0 using the [itemTemplate](/5.35.0/react/select/api#template-itemTemplate) option. Read the [Templating](/react/select/templating) section for more details.
...
We renamed the `showOnTap` option to [showOnClick](/react/select/api#opt-showOnClick) and the `showOnOverlayTap` option to [showOnOverlayClick](/5.35.0/react/select/api#opt-showOnOverlayClick) for the sake of simplicity and to make it more obvious.
