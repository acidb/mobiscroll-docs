...
description: 'Customize the input trigger for the Mobiscroll Select in JavaScript — use custom anchors, inline rendering, or custom value display.'
---
...
You can use any custom input with the select component using the `inputElement` [option](/5.35.0/javascript/select/api#opt-inputElement). Passing another element to it, the default input won't be rendered. The passed element will get the event handlers to open the picker, and also the selected value's text will be written to it as well. This option will only work with the directive version of the component.
