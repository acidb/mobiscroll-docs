...
scripts/               Build utilities
  strip-jsx.js         Post-build: removes MDX/JSX from LLM context files, and reconstructs
                       plain-text content for components the llms plugin strips down to
                       nothing (see the `{/* llms:TYPE */}` marker table in writing-docs.md)
...
- Components like `<DocsLink>`, `<Parameter>`, `<CodeBlock>` with `{props.framework}`, and `<ImgComparisonSlider>` need an `{/* llms:TYPE */}` marker or their content silently disappears from llms output — see `writing-docs.md`
