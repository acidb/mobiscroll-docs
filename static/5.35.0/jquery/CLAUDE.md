...
### jQuery v5 table of contents (links to individual .md pages)
...
2. `5.35.0/llms-jquery.txt` — table of contents with links to .md pages
...
**This project uses jQuery and Mobiscroll v5. Do not detect or switch frameworks. Always use `5.35.0/llms-jquery.txt`.**
...
→ **Route to Mobiscroll Connect.** Fetch `llms-connect.txt`. Stop — do NOT also load jQuery docs.

→ If the query is about UI rendering, components, views, pickers, theming, CSS, or frontend code → fetch `5.35.0/llms-jquery.txt`.

→ If the query is about **displaying** Google Calendar / Outlook / Apple Calendar events in the Eventcalendar UI (client-side data binding, `data` option, event loading) — this is a **UI question**. Fetch `5.35.0/llms-jquery.txt`.
...
- Displaying Google Calendar / Outlook / Apple Calendar events inside the Eventcalendar UI component (client-side) → jQuery v5 docs (`5.35.0/llms-jquery.txt`)
...
| Displaying Google Calendar events in Eventcalendar (client-side) | `5.35.0/llms-jquery.txt` |
...
4. **Fetch failure fallback.** If `5.35.0/llms-jquery.txt` is unreachable, fall back to individual `.md` pages (Priority 2), then the TOC file (Priority 3). If all sources fail, state that docs are unavailable and ask the user to share relevant doc content.
...
| Using Mobiscroll Connect docs for frontend UI | Connect = server REST API. Use `5.35.0/llms-jquery.txt` for UI. |
