...
| File | Format | For |
|:---|:---|:---|
| <DocsLink path="mobiscroll-jquery.mdc" download /> | Cursor rule file | Cursor, GitHub Copilot |
| `mobiscroll-ui/SKILL.md` + `docs/jquery/SKILL.md` + `mobiscroll-ui-theming/SKILL.md` | Claude Code skills | Cursor, GitHub Copilot, Claude Code (via plugin) |
...
|:---|:---|:---|
| **Cursor** | `llms-jquery-full.txt` via @docs | `.mdc` file or `SKILL.md` files |
...
- <DocsLink path="docs/jquery/SKILL.md" download>mobiscroll-ui-jquery SKILL.md (jQuery conventions)</DocsLink>
...
Download the <DocsLink path="mobiscroll-jquery.mdc" download><code>mobiscroll-jquery.mdc</code></DocsLink> file and place it at the root of your project, or copy [its content](#rules-files-mdc) into your `.github/` instruction files:
...
**Fix:** Verify that you have the correct `.mdc` file in place. For jQuery, use `mobiscroll-jquery.mdc`, not a different framework's `.mdc` file. In Cursor, check that the registered @docs source points to `llms-jquery-full.txt`.
...
| jQuery conventions skill | <DocsLink path="docs/jquery/SKILL.md" download /> |
