...
We renamed the `showOnTap` option to [showOnClick](/jquery/select/api#opt-showOnClick) and the `showOnOverlayTap` option to [showOnOverlayClick](/5.34.0/jquery/select/api#opt-showOnOverlayClick) for the sake of simplicity and to make it more obvious.
