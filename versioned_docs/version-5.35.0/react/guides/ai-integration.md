...
|:---|:---|:---|:---|
| **Cursor** | `llms-react.txt` via @docs | `.mdc` file | — |
| **GitHub Copilot** | `.mdc` file (contains doc URLs) | `.mdc` file | — |
| **Claude Code** | `llms-react.txt` | `CLAUDE.md` | `CLAUDE.md` |
...
| Framework | Name | URL |
|:---|:---|:---|
| React | Mobiscroll React | <DocsUrl path="5.35.0/llms-react.txt" /> |
...
Claude Code will fetch `llms-react.txt` automatically based on the detected framework. No manual registration is needed.
...
**Fix:** Verify that you have the correct `.mdc` file in place. For React, use `mobiscroll-react.mdc`, not a different framework's `.mdc` file. In Cursor, check that the registered @docs source points to `5.35.0/llms-react.txt`.
