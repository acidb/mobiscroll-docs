...
Mobiscroll uses the [`resources`](https://mobiscroll.com/docs/vue/eventcalendar/timeline#opt-resources) array with `id` and `name` properties.
...
- `scheduler.parse(data, format)` - loads events directly from a vue object. Once loaded, Scheduler automatically renders the events into the current view.
