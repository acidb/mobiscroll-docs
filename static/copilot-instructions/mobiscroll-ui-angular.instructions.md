...
Always verify available template inputs via the `getComponentSchema` MCP tool (request only the entities you need by `names` after reviewing the index) before writing
