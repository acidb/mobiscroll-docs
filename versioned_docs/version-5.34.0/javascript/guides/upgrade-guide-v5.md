...
We slightly changed the structure of the [marked](/javascript/eventcalendar/api#opt-marked), [labels](/javascript/eventcalendar/api#opt-labels) and [colors](/javascript/eventcalendar/api#opt-colors) objects. To specify the date, use the `date` property, or the `start` and `end` properties for multiple days, and use the `recurring` property to specify multiple occurrences.
...
We renamed the `showOnTap` option to [showOnClick](/javascript/select/api#opt-showOnClick) and the `showOnOverlayTap` option to [showOnOverlayClick](/5.34.0/javascript/select/api#opt-showOnOverlayClick) for the sake of simplicity and to make it more obvious.
