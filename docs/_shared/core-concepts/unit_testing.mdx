...
In the case of the Datepicker, we might not need a workaround like this. By default the Datepicker renders an input element with a label. The [@testing-library](https://testing-library.com/) gives us tools that can be used to query elements the user can perceive on the screen.
...
The [@testing-library](https://testing-library.com/) provides easy to use functions to simulate events. In the following example we use a click event to open a Datepicker and check whether the picker elements are in the DOM.
...
Events can be fired using the `fireEvent` object imported from the [@testing-library](https://testing-library.com/).
...
In the following example we will test if the `onOpen` event was fired properly. There is are specific methods in the [@testing-library](https://testing-library.com/) that allow us to test this. Whether a function was called or not can be tested using the testing framework underneath the [@testing-library](https://testing-library.com/) which is Jest.

:::info
While the [@testing-library](https://testing-library.com/) can be used with other testing frameworks, they also recommend using Jest.
:::
