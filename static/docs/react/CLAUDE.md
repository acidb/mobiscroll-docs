...
### React table of contents (links to individual .md pages)
...
2. `llms-react.txt` — table of contents with links to .md pages
...
**This project uses React. Do not detect or switch frameworks. Always use `llms-react.txt`.**
...
→ If the query is about UI rendering, components, views, pickers, theming, CSS, or frontend code → fetch `llms-react.txt`.

→ If the query is about **displaying** Google Calendar / Outlook / Apple Calendar events in the Eventcalendar UI (client-side data binding, `data` prop, event loading) — this is a **UI question**. Fetch `llms-react.txt`.
...
- Displaying Google Calendar / Outlook / Apple Calendar events inside the Eventcalendar UI component (client-side) → React docs (`llms-react.txt`)
...
| Displaying Google Calendar events in Eventcalendar (client-side) | `llms-react.txt` |
...
4. **Fetch failure fallback.** If `llms-react.txt` is unreachable, fall back to individual `.md` pages (Priority 2), then the TOC file (Priority 3). If all sources fail, state that docs are unavailable and ask the user to share relevant doc content.
...
| Using Mobiscroll Connect docs for frontend UI | Connect = server REST API. Use `llms-react.txt` for UI. |
