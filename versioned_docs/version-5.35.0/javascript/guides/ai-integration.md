...
|:---|:---|:---|:---|
| **Cursor** | `llms-javascript.txt` via @docs | `.mdc` file | — |
| **GitHub Copilot** | `.mdc` file (contains doc URLs) | `.mdc` file | — |
| **Claude Code** | `llms-javascript.txt` | `CLAUDE.md` | `CLAUDE.md` |
...
| Framework | Name | URL |
|:---|:---|:---|
| JavaScript | Mobiscroll JavaScript | <DocsUrl path="5.35.0/llms-javascript.txt" /> |
...
Claude Code will fetch `llms-javascript.txt` automatically based on the detected framework. No manual registration is needed.
...
**Fix:** Verify that you have the correct `.mdc` file in place. For JavaScript, use `mobiscroll-javascript.mdc`, not a different framework's `.mdc` file. In Cursor, check that the registered @docs source points to `5.35.0/llms-javascript.txt`.
